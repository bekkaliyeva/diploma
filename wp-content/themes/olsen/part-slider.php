<?php if( get_theme_mod( 'home_slider_show', 1 ) == 1 ): ?>
	<?php
		...
		$attributes = sprintf( 'data-autoplay="%s" data-autoplayspeed="%s" data-fade="%s" data-carousel="%s"',
			...
			esc_attr( get_theme_mod( 'home_slider_fullwidth_carousel', 0 ) )
		);

		$fullwidth = get_theme_mod( 'home_slider_fullwidth', 0 ) === 1;
		$carousel  = get_theme_mod( 'home_slider_fullwidth_carousel', 0 ) === 1;

		$slider_image   = 'ci_slider';
		$slider_classes = 'slider-contained';
		if ( $carousel ) {
			$slider_classes = 'slider-carousel';
		} elseif ( $fullwidth ) {
			$slider_classes = 'slider-fullwidth';
			$slider_image   = 'ci_slider_fullwidth';
		}

	?>
	<?php if( $args !== false && $q !== false && $q->have_posts() ): ?>

	<?php if ( ! $fullwidth && ! $carousel ) : ?>
		<div class="container">
			<div class="row">
				<div class="col-12">
	<?php endif; ?>

		<div id="home-<?php echo esc_attr( $slider_classes ); ?>" class="home-slider <?php echo esc_attr( $slider_classes ); ?>" <?php echo $attributes; ?>>
			<?php while( $q->have_posts() ): $q->the_post(); ?>
				<div class="slide">
					...
					<div class="slide-content">
						...
						<a href="<?php the_permalink(); ?>" class="read-more"><?php _e( 'Continue Reading', 'olsen' ); ?></a>
					</div>
