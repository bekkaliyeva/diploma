...
if ( ! class_exists( 'Olsen_Onboarding_Page' ) ) {
	return;
}
...
class Olsen_Onboarding_Page {
	...
	public function init( $data ) {
		if ( ! empty( $data ) && is_array( $data ) && true === $data['show_page'] ) {
			...
			if ( ! function_exists( 'olsen_get_theme_variations' ) ) {
	...
	public function page_setup() {
		...
		add_action( 'wp_ajax_olsen_dismiss_onboarding', array( $this, 'dismiss_onboarding' ) );
		add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_admin_styles' ) );
		add_action( 'admin_menu', array( $this, 'register' ) );
		add_action( 'wp_ajax_install_olsen_plugin', array( $this, 'install_plugin' ) );
		add_action( 'wp_ajax_olsen_activate_variation', array( $this, 'activate_variation' ) );
		add_action( 'wp_ajax_olsen_reset_theme_mods', array( $this, 'reset_theme_mods' ) );
	}
	...
	public function onboarding_notice() {
		...
		<div class="notice notice-info is-dismissible">
			<div class="olsen-onboarding-notice">
				<p>
					<?php
						/* translators: %1$s is the theme name. %2$s URL to onboarding page. */
						echo wp_kses( sprintf( __( 'Welcome to %1$s. Check out the <a href="%2$s">onboarding page</a> to get things started.', 'olsen' ),
							$this->theme_name,
		...
		wp_enqueue_script( 'olsen-onboarding-notice', get_theme_file_uri( '/inc/onboarding/js/onboarding-notice.js' ), array(), $this->theme_version, true );

		$settings = array(
			'ajaxurl'       => admin_url( 'admin-ajax.php' ),
			'dismiss_nonce' => wp_create_nonce( 'olsen-dismiss-onboarding' ),
		);
		wp_localize_script( 'olsen-onboarding-notice', 'olsen_Onboarding', $settings );
	}
	...
	public function dismiss_onboarding() {
		check_ajax_referer( 'olsen-dismiss-onboarding', 'nonce' );
	...
	public function enqueue_admin_styles() {
		...
		wp_enqueue_style( 'olsen-onboarding', get_theme_file_uri( '/inc/onboarding/css/onboarding-styles.css' ), array(), $this->theme_version );

		wp_enqueue_script( 'olsen-onboarding', get_theme_file_uri( '/inc/onboarding/js/onboarding.js' ), array(
			'plugin-install',
			'updates',
		), $this->theme_version, true );

		wp_localize_script(
			'olsen-onboarding', 'olsen_onboarding', array(
				'onboarding_nonce'        => wp_create_nonce( 'onboarding_nonce' ),
				'ajaxurl'                 => admin_url( 'admin-ajax.php' ),
				'template_directory'      => get_template_directory_uri(),
				'activating_text'         => esc_html__( 'Activating', 'olsen' ),
				'activate_text'           => esc_html__( 'Activate', 'olsen' ),
				'installing_text'         => esc_html__( 'Installing...', 'olsen' ),
				'activate_variation_text' => esc_html__( 'Activate variation', 'olsen' ),
				'deleting_text'           => esc_html__( 'Deleting...', 'olsen' ),
				'reset_mods_confirm_text' => esc_html__( 'Are you sure you want to delete your theme customizations?', 'olsen' ),
			)
		);
	}
	...
	public function render_page() {
		...
		if ( defined( 'OLSEN_WHITELABEL' ) && OLSEN_WHITELABEL ) {
			...
			$logo_url = ! empty( $this->data['logo_url'] ) ? $this->data['logo_url'] : 'https://www.cssigniter.com/themes/' . OLSEN_NAME . '/';
		}
		?>
		<div class="wrap about-wrap olsen-onboarding-wrap full-width-layout">
			<h1><?php echo esc_html( $title ); ?></h1>

			<?php if ( ! empty( $this->data['description'] ) ) : ?>
				<p class="about-text"><?php echo wp_kses( $this->data['description'], olsen_get_allowed_tags( 'guide' ) ); ?></p>
			<?php endif; ?>
	...
	public function theme_variations() {
		?><h3><?php esc_html_e( 'Select a theme variation. You can always come back and pick another.', 'olsen' ); ?></h3><?php

		if ( ! current_user_can( 'edit_theme_options' ) ) :
			?><p><?php echo wp_kses( __( 'You do not have sufficient permissions to install plugins, please contact your administrator.', 'olsen' ), olsen_get_allowed_tags( 'guide' ) ); ?></p><?php
			return false;
		endif;

		?><div class="olsen-onboarding-list"><?php
			$variations = $this->data['theme_variations_page']['variations'];
		...
		if ( $reset_button ) {
			...
			<div class="reset-theme-mods-wrap">
				<p><?php esc_html_e( "Your existing theme customizations made via the Customizer (theme mods) may prevent you from viewing the selected variation's defaults correctly. In this case, you might want to delete your customizations.", 'olsen' ); ?></p>
				<p><em><?php esc_html_e( 'WARNING: Deleting your customizations cannot be undone. Make sure you keep a backup before deleting your customizations.', 'olsen' ); ?></em></p>
				<p><a href="#" class="button reset-theme-mods"><?php esc_html_e( 'Delete customizations', 'olsen' ); ?></a></p>
			</div>
			<?php
		}
	}
	...
	public function get_variation_boxes( $variations ) {
		if ( ! function_exists( 'olsen_get_theme_variation' ) ) {
			return;
		}

		$current_variation = olsen_get_theme_variation();
		...
		foreach ( $variations as $slug => $variation ) {
			...
			?>
			<div class="col">
				<div class="olsen-onboarding-box olsen-variation <?php echo esc_attr( $enabled_class ); ?>">
					...
					<?php if ( ! empty( $variation['description'] ) ) : ?>
						<p class="box-description"><?php echo wp_kses( $variation['description'], olsen_get_allowed_tags( 'guide' ) ); ?></p>
					<?php endif; ?>

					<p><a href="#" class="button activate-variation" data-variation-slug="<?php echo esc_attr( $slug ); ?>"><?php esc_html_e( 'Activate variation', 'olsen' ); ?></a></p>
				</div>
			</div>
			<?php
		}
	...
	public function required_plugins() {
		?><h3><?php esc_html_e( 'The following plugins are required. Please install and activate them.', 'olsen' ); ?></h3><?php
	...
	public function recommended_plugins() {
		?><h3><?php
			...
			echo esc_html( sprintf( __( 'The following plugins are not required but they will provide additional functionality to %s.', 'olsen' ), $this->theme_name ) );
		?></h3><?php
	...
	public function sample_content() {
		...
		if ( ! empty( $plugins ) ) {
			?>
			<h3><?php esc_html_e( 'Plugins required for sample content. Please install and activate.', 'olsen' ); ?></h3>
			<p><?php esc_html_e( 'While these plugins are not required for the theme to work, they are needed to ensure the sample content is correctly imported. You can deactivate/remove them if you want, after the sample content is imported.', 'olsen' ); ?></p>

			<div class="olsen-onboarding-list">
				<?php
					$actions = $this->get_plugin_action( $plugins );
		...
		?><h3><?php esc_html_e( 'Import our sample content.', 'olsen' ); ?></h3><?php

		$check = $this->get_plugin_action( array( 'one-click-demo-import' => __( 'One Click Demo Import', 'olsen' ) ) );

		if ( in_array( $check['one-click-demo-import'], array( 'install-plugin', 'activate' ), true ) ) {
			?>
			<div class="olsen-onboarding-box olsen-onboarding-box-warning">
				<h4 class="box-title"><?php esc_html_e( 'Please note:', 'olsen' ); ?></h4>
				<p><?php echo wp_kses( __( 'You need to install and activate <strong>One Click Demo Import</strong> before proceeding.', 'olsen' ), olsen_get_allowed_tags( 'guide' ) ); ?></p>
			</div>
			<?php
		} else {
			?>
			<div class="olsen-onboarding-box olsen-onboarding-box-success">
				<h4 class="box-title"><?php esc_html_e( 'Good to go!', 'olsen' ); ?></h4>
				<p><?php esc_html_e( 'Now you can import the sample content and have your theme set up like the demo using the One Click Demo Import Plugin.', 'olsen' ); ?></p>
				<p><a class="button button-primary" href="<?php echo esc_url( get_admin_url( '', 'themes.php?page=pt-one-click-demo-import' ) ); ?>"><?php esc_html_e( 'Get Started', 'olsen' ); ?></a></p>
			</div>
			<?php
		}
	...
	public function support() {
		...
		?>
		<h3><?php esc_html_e( 'Here are a few useful links to get you started.', 'olsen' ); ?></h3>
		<div class="olsen-onboarding-list">
			<?php
				...
				foreach ( $sections as $section_id => $section ) {
					?>
					<div class="col">
						<div class="olsen-onboarding-box olsen-support-tab-box-<?php echo esc_attr( $section_id ); ?>">
							<h4 class="box-title"><a href="<?php echo esc_url( $section['link_url'] ); ?>" target="_blank"><?php echo esc_html( $section['title'] ); ?></a></h4>
							<?php if ( ! empty( $section['description'] ) ) : ?>
								<p><?php echo wp_kses( $section['description'], olsen_get_allowed_tags( 'guide' ) ); ?></p>
							<?php endif; ?>
	...
	public function get_plugin_boxes( $plugins, $actions ) {
		foreach ( $actions as $slug => $action ) {
			...
			if ( 'upload-plugin' === $action ) {
				...
				<div class="col">
					<div class="olsen-onboarding-box olsen-onboarding-box-warning">
						...
						<?php if ( ! empty( $plugins[ $slug ]['description'] ) ) : ?>
							<p class="box-description"><?php echo wp_kses( $plugins[ $slug ]['description'], olsen_get_allowed_tags( 'guide' ) ); ?></p>
						<?php endif; ?>
						...
						<div class="plugin-card-<?php echo esc_attr( $slug ); ?>">
							...
							<?php
								...
								echo esc_html( sprintf( __( 'The %s plugin was not found. Click to install and activate.', 'olsen' ), $plugins[ $slug ]['title'] ) );
							?>
							</p>
							<p><a href="<?php echo esc_url( $link ); ?>" class="ajax-install-plugin button" data-plugin-slug="<?php echo esc_attr( $slug ); ?>"><?php esc_html_e( 'Install &amp; activate ', 'olsen' ); ?></a></p>
						</div>
					</div>
				</div>
				<?php
				...
				<div class="col">
					<div class="olsen-onboarding-box olsen-onboarding-box-warning">
						...
						<div class="plugin-card-<?php echo esc_attr( $slug ); ?>">
							...
							<p><a data-plugin-slug="<?php echo esc_attr( $slug ); ?>" href="<?php echo esc_url( $link ); ?>" class="install-now button"><?php esc_html_e( 'Install &amp; activate ', 'olsen' ); ?></a></p>
						</div>
					</div>
				</div>
				<?php
				...
				?>
				<div class="col">
					<div class="olsen-onboarding-box olsen-onboarding-box-info">
						<h4 class="box-title"><?php echo esc_html( $plugins[ $slug ]['title'] ); ?></h4>
						...
						<div class="plugin-card-<?php echo esc_attr( $slug ); ?>">
							...
							<?php
								/* translators: %s is the plugin name. */
								echo esc_html( sprintf( __( 'The %s plugin is installed but not active. Click to activate.', 'olsen' ), $plugins[ $slug ]['title'] ) );
							?>
							</p>
							<p><a data-plugin-slug="<?php echo esc_attr( $slug ); ?>" href="<?php echo esc_url( $link ); ?>" class="activate-now button button-primary"><?php esc_html_e( 'Activate ', 'olsen' ); ?></a></p>
						</div>
					</div>
				</div>
				<?php
				...
				<div class="col">
					<div class="olsen-onboarding-box olsen-onboarding-box-success">
						...
						<?php
							/* translators: %s is the plugin name. */
							echo esc_html( sprintf( __( '%s is installed and activated.', 'olsen' ), $plugins[ $slug ]['title'] ) );
						?>
						</p>
					</div>
				</div>
	...
	public function install_plugin() {
		if ( ! current_user_can( 'upload_plugins' ) ) {
			wp_die( esc_html__( 'You do not have sufficient permissions to install plugins on this site.', 'olsen' ) );
		}
		...
		$title    = sprintf( __( 'Installing Plugin from URL: %s', 'olsen' ), esc_html( $plugin_source_url ) );
	...
	public function activate_variation() {
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			wp_die( esc_html__( 'You do not have sufficient permissions to change theme options on this site.', 'olsen' ) );
		}
		...
		if ( array_key_exists( $variation, olsen_get_theme_variations() ) ) {
	...
	public function reset_theme_mods() {
		...
		$defaults   = olsen_customizer_defaults();
	...
	public function default_data() {
		...
		return array(
			...
			'menu_title'               => sprintf( __( 'About %s', 'olsen' ), $theme->get( 'Name' ) ),
			// Optional. The text to be displayed in the page's title tag. If empty, defaults to "About theme_name"
			/* translators: %s is the theme name. */
			'page_title'               => sprintf( __( 'About %s', 'olsen' ), $theme->get( 'Name' ) ),
			...
			'title'                    => __( 'Welcome to :theme_name:! - Version :theme_version:', 'olsen' ),
			...
			'logo_show'                => defined( 'OLSEN_WHITELABEL' ) && OLSEN_WHITELABEL ? false : true,
			...
			// Required. The logo's link URL. Defaults to 'https://www.cssigniter.com/themes/' . OLSEN_NAME . '/'
			'logo_url'                 => 'https://www.cssigniter.com/themes/' . OLSEN_NAME . '/',
			...
			'tabs'                     => array(
				'theme_variations'    => __( 'Theme Variations', 'olsen' ),
				'required_plugins'    => __( 'Required Plugins', 'olsen' ),
...
//						'title'       => __( 'Variation Title', 'olsen' ),
...
//					),
			...
			'support_page'             => array(
				'sections' => array(
					'documentation' => array(
						'title'       => __( 'Theme Documentation', 'olsen' ),
						'description' => __( "If you don't want to import our demo sample content, just visit this page and learn how to set things up individually.", 'olsen' ),
						'link_url'    => 'https://www.cssigniter.com/docs/' . OLSEN_NAME . '/',
					),
					'kb'            => array(
						'title'       => __( 'Knowledge Base', 'olsen' ),
						'description' => __( 'Browse our library of step by step how-to articles, tutorials, and guides to get quick answers.', 'olsen' ),
						'link_url'    => 'https://www.cssigniter.com/docs/knowledgebase/',
					),
					'support'       => array(
						'title'       => __( 'Request Support', 'olsen' ),
						'description' => __( 'Got stuck? No worries, just visit our support page, submit your ticket and we will be there for you within 24 hours.', 'olsen' ),
