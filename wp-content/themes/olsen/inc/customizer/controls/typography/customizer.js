(function ( $ ) {
	...
	wp.customize.controlConstructor[ 'olsen-typography' ] = wp.customize.Control.extend( {
		ready: function () {
			var control = this;
			...
			$fontSelect.on( 'change', function () {
				...
				var font                     = _.findWhere( olsen_gfonts.fonts, { family: fontFamily } );
			...
			$lineHeightInput.on( 'change keyup', function () {
				var $this      = $( this );
				var $wrapper   = $this.parents( '.olsen-control-range' );
				var value      = $this.val();
				var breakpoint = $this.parents( '[data-breakpoint]' ).data( 'breakpoint' );
		...
		displayDeviceControls: function ( device ) {
			...
			var sections = control.container.find( '.olsen-typography-control-group-wrap' );
