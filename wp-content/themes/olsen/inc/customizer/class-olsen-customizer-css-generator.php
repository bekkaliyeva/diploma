<?php
class Olsen_Customizer_CSS_Generator {

	/**
	 * Holds a copy of itself, so it can be referenced by the class name.
	 *
	 * @var Olsen_Customizer_CSS_Generator
	 */
	 ...
	 *
	 * @return Olsen_Customizer_CSS_Generator
	 */
	...
	public function get_array( $breakpoint ) {
		...
		return apply_filters( 'olsen_customizer_css_generator_get_array', $return, $breakpoint );
	}

	public function get( $breakpoint ) {
		$return = implode( PHP_EOL, $this->get_array( $breakpoint ) );

		return apply_filters( 'olsen_customizer_css_generator_get', $return, $breakpoint );
	}
	...
	public function add( $breakpoint, $css ) {
		...
		$css = trim( $css );
		$css = apply_filters( 'olsen_customizer_css_generator_add', $css, $breakpoint );
	...
	public function generate_typography_rules( $values, $fallback_stack = '' ) {
		...
		if ( isset( $values['family'] ) && ! $this->value_is_empty( $values['family'] ) ) {
			...
			$stack = apply_filters( 'olsen_customizer_css_generator_generate_typography_stack', $stack, $values, $fallback_stack );
			...
			$all_stack = apply_filters( 'olsen_customizer_css_generator_generate_typography_all_stack', $all_stack, $values, $stack, $fallback_stack );
	...
	public function get_google_fonts_url() {
		...
		foreach ( $this->typography_controls as $option_name => $default ) {
			if ( 'placeholder_preview_font' !== $option_name && ! apply_filters( 'olsen_customizer_css_generator_add_font_to_url', true, $option_name, $default ) ) {
				continue;
			}
		...
		foreach ( $fonts as $font_name => $font_weights ) {
			...
			$standard_weights = apply_filters( 'olsen_customizer_css_generator_standard_gfont_weights', array(
				'400',
				'400i',
				'700',
			) );
			...
			$enqueue_weights = apply_filters( 'olsen_customizer_css_generator_enqueue_gfont_weights', $enqueue_weights, $font_name, $available_weights, $selected_weights, $standard_weights );
