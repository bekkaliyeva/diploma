<?php
add_action( 'after_setup_theme', 'olsen_setup_helpers_post_meta' );
function olsen_setup_helpers_post_meta() {
	add_image_size( 'ci_featgal_small_thumb', 100, 100, true );

	add_action( 'wp_ajax_olsen_featgal_AJAXPreview', 'olsen_featgal_AJAXPreview' );
}

//
// Various wrapping functions for easier custom fields creation.
//
function olsen_prepare_metabox( $post_type ) {
	wp_nonce_field( basename( __FILE__ ), $post_type . '_nonce' );
}

function olsen_can_save_meta( $post_type ) {
	global $post;
...
function olsen_metabox_gallery( $gid = 1 ) {
	global $post;
	$post_id = $post->ID;

	olsen_featgal_print_meta_html( $post_id, $gid );
}

function olsen_metabox_gallery_save( $POST, $gid = 1 ) {
	global $post;
	$post_id = $post->ID;

	olsen_featgal_update_meta( $post_id, $POST, $gid );
}

function olsen_metabox_input( $fieldname, $label, $params = array() ) {
	global $post;
...
function olsen_metabox_textarea( $fieldname, $label, $params = array() ) {
	global $post;
...
function olsen_metabox_dropdown( $fieldname, $options, $label, $params = array() ) {
	global $post;
...
function olsen_metabox_radio( $fieldname, $optionname, $optionval, $label, $params = array() ) {
	global $post;
...
function olsen_metabox_checkbox( $fieldname, $value, $label, $params = array() ) {
	global $post;
...
function olsen_metabox_open_tab( $title ) {
	?>
...
function olsen_metabox_close_tab() {
	?>
		</div>
	</div>
	<?php
...
function olsen_metabox_open_collapsible( $title ) {
	?>
	<div class="postbox" style="margin-top:20px">
		<div class="handlediv" title="<?php esc_attr_e( 'Click to toggle', 'olsen' ); ?>"><br></div>
		<h3 class="hndle"><?php echo esc_html( $title ); ?></h3>
		<div class="inside">
	<?php
}

function olsen_metabox_close_collapsible() {
	?>
		</div>
	</div>
	<?php
...
function olsen_metabox_guide( $strings, $params = array() ) {
	$defaults = array(
...
function olsen_bind_metabox_to_page_template( $metabox_id, $template_file, $js_var ) {
    ...
    (function($) {
	    ...
	    $(window).on( 'load', function() {
			...
			if ( template_box.length > 0 ) {
				...
				template_box.on('change', function() {
					if ( $.inArray( template_box.val(), {$js_var}_template ) > -1 ) {
						{$js_var}.show();
...
ENDJS;
	...
	olsen_add_inline_js( $js, sanitize_key( 'metabox_template_' . $metabox_id . '_' . $template_file ) );
}

function olsen_bind_metabox_to_post_format( $metabox_id, $post_format, $js_var ) {
	...
	(function($) {
		...
		$(window).on( 'load', function() {
			...
			if ( formats_box.length > 0 ) {
				...
				formats_box.on('change', function() {
...
ENDJS;
	...
	olsen_add_inline_js( $js, sanitize_key( 'metabox_format_' . $metabox_id . '_' . $post_format ) );
}
 ...
 * @return void
 */
function olsen_featgal_print_meta_html( $post_id = false, $gid = 1 ) {
	if ( $post_id == false ) {
		global $post;
		$post_id = $post->ID;
	}
	...
	<div class="ci-media-manager-gallery">
		<input type="button" class="ci-upload-to-gallery button" value="<?php _e( 'Add Images', 'olsen' ); ?>"/>
		...
		<p><label class="ci-upload-to-gallery-random"><input type="checkbox" name="ci_featured_gallery_rand_<?php echo esc_attr( $gid ); ?>" value="rand" <?php checked( $rand, 'rand' ); ?> /> <?php _e( 'Randomize order', 'olsen' ); ?></label></p>
		<div class="ci-upload-to-gallery-preview group">
			<?php
				$images = olsen_featgal_get_images( $ids );
				if ( $images !== false and is_array( $images ) ) {
					foreach ( $images as $image ) {
						?>
						<div class="thumb">
							<img src="<?php echo esc_url( $image['url'] ); ?>" data-img-id="<?php echo esc_attr( $image['id'] ); ?>">
							<a href="#" class="close media-modal-icon" title="<?php echo esc_attr( __( 'Remove from gallery', 'olsen' ) ); ?>"></a>
						</div>
						<?php
					}
				}
			?>
			<p class="ci-upload-to-gallery-preview-text"><?php _e( 'Your gallery images will appear here', 'olsen' ); ?></p>
		</div>
	</div>
	<?php
}
 ...
 */
function olsen_featgal_update_meta( $post_id, $POST, $gid = 1 ) {
	if ( absint( $post_id ) < 1 ) {
		return false;
	}
...
function olsen_featgal_get_ids( $post_id = false, $gid = 1 ) {
	if ( $post_id == false ) {
		global $post;
		$post_id = $post->ID;
	} else {
...
function olsen_featgal_get_attachments( $post_id = false, $gid = 1, $extra_args = array() ) {
	if ( $post_id == false ) {
		global $post;
		$post_id = $post->ID;
	} else {
 ...
 * Hooked to wp_ajax_olsen_featgal_AJAXPreview for AJAX updating of the galleries' previews.
 */
function olsen_featgal_AJAXPreview() {
	$ids  = $_POST['ids'];
	$urls = olsen_featgal_get_images( $ids );
	if ( $urls === false ) {
		echo 'FAIL';
 ...
 * @return array|bool
 */
function olsen_featgal_get_images( $csv = false ) {
	$ids = explode(',', $csv);
	$ids = array_filter($ids);
	...
	if ( count( $ids ) > 0 ) {
		...
		foreach ( $ids as $id ) {
			$thumb_file = olsen_get_image_src( $id, 'ci_featgal_small_thumb' );
			...
			if ( olsen_substr_right( $file, strlen( $size ) ) == $size ) {
				$file = $thumb_file;
			} else {
				$file = olsen_get_image_src( $id, 'thumbnail' );
			}
