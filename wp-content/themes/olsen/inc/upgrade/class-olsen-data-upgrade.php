...
class Olsen_Data_Upgrade {
	...
	/**
	 * @var Olsen_Data_Upgrade_Background_Process
	 */
	private $bg_process;
	...
	public function __construct( $debug = false ) {
		...
		$this->db_version    = get_option( 'olsen_theme_db_version', false );
		$this->is_upgrading  = get_option( 'olsen_upgrade_in_progress', false );
		...
		$this->default_log_message = __( 'Updating data...', 'olsen' );

		$this->log        = new Olsen_Data_Upgrade_Log( 'olsen_upgrade_log_message', $this->default_log_message, $debug );
		$this->bg_process = new Olsen_Data_Upgrade_Background_Process( $this );

		add_action( 'admin_init', array( $this, 'admin_init' ) );
		add_action( 'wp_ajax_olsen_data_upgrade_get_status', array( $this, 'ajax_get_status' ) );
	}

	public function admin_init() {
		if ( is_admin() && ( $this->needs_upgrade() || $this->is_upgrading() ) ) {
			wp_enqueue_script( 'olsen-data-upgrade', get_theme_file_uri( '/inc/upgrade/js/upgrade.js' ), array( 'jquery' ), olsen_asset_version(), true );
			wp_localize_script( 'olsen-data-upgrade', 'olsen_data_upgrade', array(
				'ajax_url'        => admin_url( 'admin-ajax.php' ),
				'text_done'       => __( 'The theme finished upgrading data.', 'olsen' ),
				'update_interval' => apply_filters( 'olsen_data_upgrade_ajax_update_interval', 1 ) * 1000,
			) );
	...
	public function admin_notice() {
		?>
		<div id="olsen-data-upgrade" class="notice-warning notice">
			<p class="primary"><?php echo wp_kses( __( 'The theme updates data in the background. Please wait.', 'olsen' ), olsen_get_allowed_tags( 'guide' ) ); ?></p>
	...
	public function set_is_upgrading( $is_upgrading ) {
		if ( $is_upgrading ) {
			...
			update_option( 'olsen_upgrade_in_progress', true );
		} else {
			if ( $this->is_upgrading ) {
				// Only touch the database when the state changes. Avoids unnecessary db writes.
				delete_option( 'olsen_upgrade_in_progress' );
			}

			$this->is_upgrading = false;
		}

		$this->log->debug( sprintf( 'Olsen_Upgrade - In progress: %s', $this->is_upgrading ? 'true' : 'false' ) );
	}
	...
	public function maybe_upgrade() {
		if ( $this->is_upgrading() ) {
			$this->log->debug( 'Olsen_Upgrade - maybe_upgrade() exits because is_upgrading()' );
			return false;
		}

		if ( ! $this->bg_process->is_queue_empty() ) {
			$this->log->debug( 'Olsen_Upgrade - maybe_upgrade() exits because not is_queue_empty()' );
			return false;
		}

		if ( ! $this->needs_upgrade() ) {
			$this->log->debug( 'Olsen_Upgrade - maybe_upgrade() exits because not needs_upgrade()' );
			$this->set_db_version( $this->get_theme_version() );
			$this->log->clear();
			return false;
		}

		// This is cleared in Olsen_Data_Upgrade_Background_Process::complete()
	...
	private function do_upgrade() {
		...
		if ( $callbacks && $next_version ) {
			foreach ( $callbacks as $callback ) {
				$this->log->debug( sprintf( 'Olsen_Upgrade - Queueing %s for version %s.', $callback, $next_version ) );

				$this->bg_process->push_to_queue( apply_filters( 'olsen_data_upgrade_callback', array(
					'callback'     => $callback,
	...
	public function set_db_version( $db_version ) {
		...
		update_option( 'olsen_theme_db_version', $this->db_version );

		$this->log->debug( sprintf( 'Olsen_Upgrade - Theme database version set to: %s.', $this->db_version ) );
	}
