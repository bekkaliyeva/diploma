jQuery( document ).ready( function( $ ) {
	...
	$( 'body' ).on( 'click', '.ci-media-button', function( e ) {
		...
		var ciMediaUpload = wp.media( {
			...
			title: bMulti == true ? olsen_PostMeta.tSelectFiles : olsen_PostMeta.tSelectFile,
			button: {
				text: bMulti == true ? olsen_PostMeta.tUseTheseFiles : olsen_PostMeta.tUseThisFile
			},
			multiple: bMulti
		} ).on( 'select', function(){
				...
				*/
				...
				if( bMulti == false ) {
					...
					if( target_preview.length > 0 ) {
						...
						var html = '<img src="' + attachment.sizes.thumbnail.url + '" /><a href="#" class="close media-modal-icon" title="' + olsen_PostMeta.tRemoveImage + '"></a>';
	...
	$( 'body' ).on( 'click', '.ci-upload-to-gallery', function( e ) {
		...
		var ciMediaGallery = wp.media( {
			frame: 'select',
			title: olsen_PostMeta.tSelectFiles,
			button: {
				text: olsen_PostMeta.tUseTheseFiles
			},
			multiple: true
			...
			olsen_featgal_AJAXUpdate( target_parent );
	...
	function olsen_featgal_UpdateIDs( preview_element ) {
		var ids = [];
	...
	function olsen_featgal_AJAXUpdate( gallery_container ) {
		...
		$.ajax( {
			type: 'post',
			url: olsen_PostMeta.ajaxurl,
			data: {
				action: 'olsen_featgal_AJAXPreview',
				ids: target_ids.val()
			},
			dataType: 'text',
			beforeSend: function() {
				target_preview.empty().html( '<p>' + olsen_PostMeta.tLoading + '</p>' );
			},
			success: function( response ) {
				if ( response == 'FAIL' ) {
					target_preview.empty().html( '<p>' + olsen_PostMeta.tPreviewUnavailable + '</p>' );
				} else {
					...
					$.each( response, function( key, value ) {
						$( '<div class="thumb"><img src="' + value.url + '" data-img-id="' + value.id + '"><a href="#" class="close media-modal-icon" title="' + olsen_PostMeta.tRemoveFromGallery + '"></a></div>' ).appendTo( target_preview );
					} );
				}
			}//success
		});//ajax
	...
	$( 'body' ).on( 'click', '.ci-media-manager-gallery .thumb a.close', function( event ) {
		...
		olsen_featgal_UpdateIDs( container );
	} );
