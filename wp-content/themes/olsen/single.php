...
<div class="row">
	...
	<div class="<?php echo $layout === 'full_wide' ? 'col-12' : 'col-lg-8 col-12' ?> <?php echo esc_attr( $layout === 'full' ? 'offset-lg-2' : '' ); ?>">
		<main id="content">
			<div class="row">
				<div class="col-12">
								...
								<?php if ( 'gallery' === get_post_format() && $gallery->have_posts() ) : ?>
									<?php if ( $gallery_type === 'tiled' ) : ?>
										<div class="entry-justified" data-height="150">
											<?php while ( $gallery->have_posts() ) : $gallery->the_post(); ?>
												<a class="jg-entry ci-lightbox" href="<?php echo esc_url( olsen_get_image_src( get_the_ID(), 'large' ) ); ?>">
													<?php $attachment = wp_prepare_attachment_for_js( get_the_ID() ); ?>
													<img src="<?php echo esc_url( olsen_get_image_src( get_the_ID(), 'ci_wide' ) ); ?>" alt="<?php echo esc_attr( $attachment['alt'] ); ?>" />
												</a>
											<?php endwhile; ?>
									...
									<?php else : ?>
										<div class="home-slider">
											<?php while ( $gallery->have_posts() ) : $gallery->the_post(); ?>
												<div class="slide">
													<a class="ci-lightbox" href="<?php echo esc_url( olsen_get_image_src( get_the_ID(), 'large' ) ); ?>">
														<?php $attachment = wp_prepare_attachment_for_js( get_the_ID() ); ?>
														<img src="<?php echo esc_url( olsen_get_image_src( get_the_ID(), $image_size ) ); ?>" alt="<?php echo esc_attr( $attachment['alt'] ); ?>" />
													</a>
												</div>
											<?php endwhile; ?>
											<?php wp_reset_postdata(); ?>
							...
							<?php if ( get_theme_mod( 'single_nextprev', 1 ) ) : ?>
								<div id="paging" class="group">
									...
									<?php if( ! empty( $next_post ) ): ?>
										<a href="<?php echo esc_url( get_permalink( $next_post ) ); ?>" class="paging-standard paging-older"><?php _e( 'Previous Post', 'olsen' ); ?></a>
									<?php endif; ?>
									<?php if( ! empty( $prev_post ) ): ?>
										<a href="<?php echo esc_url( get_permalink( $prev_post ) ); ?>" class="paging-standard paging-newer"><?php _e( 'Next Post', 'olsen' ); ?></a>
									<?php endif; ?>
								</div>
							<?php endif; ?>
