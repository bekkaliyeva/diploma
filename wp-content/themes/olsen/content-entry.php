<?php
	if ( is_tag() || is_category() ) {
		$layout = olsen_get_layout_classes( 'layout_terms' );
	} else {
		$layout = olsen_get_layout_classes( 'layout_blog' );
	}
	...
	if( in_array( $layout['layout'], array( 'small_side', '2cols_side' ) ) && is_main_query() && $wp_query->current_post == 0 ) {
		$post_class = '';
		$post_col   = $layout['layout'] === '2cols_side' ? 'col-12' : '';
		$masonry    = false;
		$is_classic = true;
	}
...
<article id="entry-<?php the_ID(); ?>" <?php post_class( 'entry ' . $post_class ); ?>>
				...
				<?php if ( $gallery_type === 'tiled' ) : ?>
					<div class="entry-justified" data-height='150'>
						<?php while ( $gallery->have_posts() ) : $gallery->the_post(); ?>
							<a class="ci-lightbox jg-entry" href="<?php echo esc_url( olsen_get_image_src( get_the_ID(), 'large' ) ); ?>">
	...
	<?php if ( 1 === get_theme_mod( 'blog_related', 0 ) && in_array( $layout['layout'], array( 'classic_2side', 'classic_2side_right', 'classic_1side', 'classic_full' ) ) ) { ?>
